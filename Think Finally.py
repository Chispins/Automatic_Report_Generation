...
def process_data(folder_path):
    ...
    try:
        ...
        # Nuevo código de procesamiento
        merged = pd.merge(devengado, categorias_codigos, how='outer', on='Cod_SIGFE')
        exported_v1 = merged.groupby(by="Cod_SIGFE")[
            ["MONTO ", "Item_en_SIGCOM", "Item SIGFE", "Subasignaciones"]].sum()
        ...
        exported_v1.loc[:, ["Cod_SIGFE", "Subasignaciones"]]
        # Nota: shape() debe ser shape sin paréntesis
        exported.shape
        ...
        # Usamos exported_v3 como base y aplicamos las transformaciones restantes
        exported = exported_v3


        columns_exported = exported.index()
        exported = columns_exported
        exported = exported[:, 1:]
        ###############################
